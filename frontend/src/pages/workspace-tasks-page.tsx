  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5">
      <section className="rounded-2xl border border-[rgba(17,17,16,0.10)] bg-white/70 backdrop-blur-sm p-4 sm:p-6">
        ...
        <div className="mt-4 inline-flex rounded-lg border border-[rgba(17,17,16,0.10)] bg-ink-50 p-1">
          <button
            onClick={() => setView('table')}
            className={`inline-flex items-center rounded-md px-3 py-1.5 text-xs font-semibold ${view === 'table' ? 'bg-white/70 backdrop-blur-sm text-ink-900 shadow-card' : 'text-ink-500'}`}
          >
          ...
          <button
            ...
            className={`inline-flex items-center rounded-md px-3 py-1.5 text-xs font-semibold ${view === 'board' ? 'bg-white/70 backdrop-blur-sm text-ink-900 shadow-card' : 'text-ink-500'}`}
          >
