...
def _load_general_context(organization) -> GeneralAgentContext:
    ...
    return GeneralAgentContext(
        ...
        allowed_topics=[str(item).strip() for item in (general_profile.get('allowed_topics') or []) if str(item).strip()],
        blocked_topics=[str(item).strip() for item in (general_profile.get('blocked_topics') or []) if str(item).strip()],
        handoff_to_sales_when=[str(item).strip() for item in (general_profile.get('handoff_to_sales_when') or []) if str(item).strip()],
        handoff_to_human_when=[str(item).strip() for item in (general_profile.get('handoff_to_human_when') or []) if str(item).strip()],
