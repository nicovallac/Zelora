  ...
  return (
    <div className="space-y-8">
      ...
      <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-500 p-8 text-white shadow-xl">
        ...
        <div className="mt-6">
          <button className="inline-flex items-center gap-2 rounded-xl bg-white px-6 py-3 text-sm font-bold text-brand-700 shadow-lg transition hover:bg-brand-50">
            Solicitar Piloto
          </button>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <h2 className="mb-1 text-xl font-bold text-slate-900">Costo real de WhatsApp</h2>
        <p className="mb-4 text-sm text-slate-500">La gran mayoría de consultas de soporte NO generan costo</p>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <tbody className="divide-y divide-emerald-100">
                <tr>
                  <td className="py-2 text-slate-600">Total afiliados activos</td>
                  <td className="py-2 text-right font-semibold text-slate-900">{whatsappExample.totalAfiliados}</td>
                </tr>
                <tr>
                  <td className="py-2 text-slate-600">Consultas estimadas al mes</td>
                  <td className="py-2 text-right font-semibold text-slate-900">{whatsappExample.consultasMes}</td>
                </tr>
                <tr>
                  <td className="py-2 text-slate-600">Mensajes por consulta (promedio)</td>
                  <td className="py-2 text-right font-semibold text-slate-900">{whatsappExample.mensajesPorConsulta}</td>
                </tr>
                <tr>
                  <td className="py-2 text-slate-600">Total mensajes intercambiados</td>
                  <td className="py-2 text-right font-semibold text-slate-900">{whatsappExample.totalMensajes}</td>
                </tr>
                <tr>
        ...
        <div>
          <p className="mb-3 font-semibold text-slate-800">¿Cuándo sí pagas? (conversaciones iniciadas por empresa)</p>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-50">
                {whatsappCategoryPrices.map((row) => (
                  <tr key={row.tipo} className="text-sm hover:bg-slate-50 transition">
                    <td className="py-2.5 pr-4 font-medium text-slate-900">{row.tipo}</td>
                    <td className="py-2.5 pr-4 font-semibold text-slate-700">{row.precio}</td>
                    <td className="py-2.5 text-slate-600">{row.ejemplo}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="space-y-4">
        <h2 className="text-xl font-bold text-slate-900">Planes recurrentes (después del piloto)</h2>
            ...
            >
              {plan.highlight && (
                <>
                  ...
                  <div className="bg-white p-5">
                    ...
                    <button className="mt-5 w-full rounded-xl bg-brand-600 py-2.5 text-sm font-bold text-white transition hover:bg-brand-700">
              ...
              {!plan.highlight && (
                <div className="bg-white p-5">
                  ...
                  {plan.period && <p className="text-sm text-slate-500">{plan.period}</p>}
                  <p className="mt-1 text-xs font-semibold text-slate-500">{plan.volume}</p>
                  <ul className="mt-4 space-y-2">
                    {plan.features.map((f) => (
                      <li key={f} className="flex items-start gap-2 text-sm text-slate-700">
                        <Check size={14} className="mt-0.5 flex-shrink-0 text-slate-400" />
                        {f}
                      </li>
                    ))}
                  </ul>
                  <button className="mt-5 w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 text-sm font-semibold text-slate-700 transition hover:bg-slate-100">
      ...
      {/* SECTION 4: Add-ons */}
      <section className="space-y-4">
        <h2 className="text-xl font-bold text-slate-900">Add-ons disponibles</h2>
        <div className="grid gap-3 sm:grid-cols-2">
          {addons.map((addon) => (
            <div key={addon.name} className="flex items-start gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="flex-1">
                ...
                <p className="mt-0.5 text-sm text-slate-500">{addon.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="rounded-2xl border border-amber-200 bg-amber-50 p-6">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          {[
            ...
            { icon: Database, text: 'Integración con sistemas internos (SISFAMILIAR, SISCOMF, ERP) para datos en tiempo real' },
            ...
            <div key={i} className="flex items-start gap-2.5 rounded-xl bg-white p-3 text-sm text-amber-900">
