...
export function InstagramPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm" style={{ height: '620px' }}>
        <div className="flex h-full">
              ...
              {(Object.keys(FILTER_LABELS) as Filter[]).map((f) => (
                <button
                  ...
                  className={`flex-1 rounded-lg py-1 text-xs font-semibold transition ${
                    filter === f ? 'bg-pink-100 text-pink-700' : 'text-slate-500 hover:bg-slate-100'
                  }`}
                >
                  {FILTER_LABELS[f]}
                </button>
              ...
              {filtered.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`flex w-full items-start gap-3 border-b border-slate-50 px-4 py-3 text-left transition hover:bg-slate-50 ${isActive ? 'bg-pink-50' : ''}`}
                  >
                    <div
            ...
            <div className="flex items-center gap-3 border-b border-slate-100 px-4 py-3">
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400">{active.intent}</span>
                </div>
              </div>
            </div>
              ...
              {active.messages.map((msg) => (
                <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-start' : 'justify-end'}`}>
                  ...
                  >
                    ...
                    <p>{msg.content}</p>
                    <p className={`mt-1 text-right text-[10px] ${msg.role !== 'user' ? 'text-white/70' : 'text-slate-400'}`}>
          ...
          <div className="hidden w-56 flex-shrink-0 flex-col gap-4 overflow-y-auto border-l border-slate-200 bg-white p-4 xl:flex">
            <div>
              <div
              ...
              <p className="text-center text-xs text-slate-500">{active.user.telefono}</p>
            </div>

            <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 text-xs space-y-2">
              <p className="font-semibold text-slate-700">Información</p>
              <div>
                <span className="text-slate-500">Tipo afiliado: </span>
                <span className="font-medium capitalize text-slate-800">{active.user.tipoAfiliado}</span>
              </div>
              <div>
                <span className="text-slate-500">Intención: </span>
                <span className="font-medium text-slate-800">{active.intent}</span>
              </div>
              <div>
                <span className="text-slate-500">Estado: </span>
                <span className="font-medium capitalize text-slate-800">{active.status.replace('_', ' ')}</span>
              </div>
            </div>

            <div className="space-y-2">
              <p className="text-xs font-semibold text-slate-700">Sentimiento</p>
              <SentimentBadge sentiment={active.sentiment} />
            </div>

            <div className="space-y-1">
              <p className="text-xs font-semibold text-slate-700">Historial de intenciones</p>
              <div className="rounded-lg border border-slate-100 bg-slate-50 p-2 text-xs text-slate-600">
                <p>• {active.intent}</p>
                <p className="text-slate-400">Conversación actual</p>
              </div>
            </div>
          </div>
        </div>
