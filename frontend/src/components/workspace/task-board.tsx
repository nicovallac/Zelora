...
export function TaskBoard({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6 gap-3">
      {columns.map((column) => (
        <div key={column} className="rounded-xl border border-[rgba(17,17,16,0.10)] bg-white/70 backdrop-blur-sm p-3">
          <p className="text-xs uppercase tracking-wide text-ink-500 font-semibold">{column}</p>
          <div className="mt-2 space-y-2">
            {tasks.filter((task) => task.status === column).map((task) => (
              <article key={task.id} className="rounded-lg border border-[rgba(17,17,16,0.08)] p-2.5 bg-ink-50">
