...
function agentCardTone(type: WorkspaceAgentType) {
  ...
  return 'border-[rgba(17,17,16,0.10)] bg-white/70 backdrop-blur-sm';
}
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-5">
            ...
            return (
              <article key={agent.id} className={`rounded-xl border p-4 ${agentCardTone(agent.type)}`}>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="rounded-full bg-white/70 backdrop-blur-sm px-2 py-1 text-[11px] font-semibold text-ink-700 border border-[rgba(17,17,16,0.10)]">
                    {agent.kpiLabel}: {agent.kpiValue}
                  </span>
                </div>

                <div className="mt-3 grid grid-cols-2 gap-2">
                  <div className="rounded-lg bg-white/70 backdrop-blur-sm border border-[rgba(17,17,16,0.10)] p-2">
                ...
                <div className="mt-3 rounded-lg border border-[rgba(17,17,16,0.10)] bg-white/70 backdrop-blur-sm p-3">
                ...
                <div className="mt-3 flex gap-2">
                  <button
                    ...
                    className="flex-1 inline-flex items-center justify-center rounded-md bg-brand-500 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-600"
                  >
                  ...
                  <button
                    ...
                    className="rounded-md border border-[rgba(17,17,16,0.10)] bg-white/70 backdrop-blur-sm px-3 py-2 text-xs font-semibold text-ink-700 hover:bg-ink-50"
                  >
                    Ver tareas
                  </button>
                </div>
      ...
      {activeAgent && (
        <aside className="fixed inset-0 z-40">
          ...
          <div className="absolute right-0 top-0 h-full w-full max-w-md bg-white/70 backdrop-blur-sm border-l border-[rgba(17,17,16,0.10)] p-4 flex flex-col">
            <div className="flex items-center justify-between">
              <div>
              ...
              {selectedTopics.map((topic) => (
                <button
                  ...
                  className={`rounded-full px-2 py-1 text-[11px] font-semibold ${
                    activeTopic === topic ? 'bg-brand-500 text-white' : 'bg-ink-100 text-ink-700'
                  }`}
                >
                  {topic}
                </button>
            ...
            <div className="mt-3 flex-1 overflow-y-auto space-y-2 rounded-lg border border-[rgba(17,17,16,0.10)] bg-ink-50 p-2">
              {messages.map((msg) => (
                <div
                  key={msg.id}
                  className={`max-w-[92%] rounded-lg px-3 py-2 text-xs ${
                    msg.role === 'agent'
                      ? 'bg-white/70 backdrop-blur-sm text-ink-700 border border-[rgba(17,17,16,0.10)]'
            ...
            <div className="mt-3 flex items-center gap-2 rounded-lg border border-[rgba(17,17,16,0.10)] px-3 py-2">
              <input
                value={prompt}
