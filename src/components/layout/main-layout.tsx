...
const NAV_SECTIONS = [
  ...
  {
    label: 'Omnicanal',
    paths: ['/omnichannel', '/demo-web', '/whatsapp', '/instagram', '/tiktok', '/app-chat'],
  },
  {
    label: 'Contenido',
  ...
  {
    label: 'E-commerce',
    paths: ['/products', '/inventory', '/orders'],
  },
  {
    label: 'AI Workspace',
    paths: ['/workspace', '/workspace/overview', '/workspace/memory', '/workspace/tasks', '/workspace/insights', '/workspace/activity', '/workspace/decisions'],
  },
  {
    label: 'Configuracion',
...
const MOCK_ORGS = [
  ...
  { id: 2, name: 'Demo Corp', plan: 'Plan Basico' },
];

const ALL_NAV = [
  { label: 'Dashboard (Operativo)', path: '/', icon: 'LayoutDashboard', audience: 'all' },
  { label: 'Omnicanal Hub', path: '/omnichannel', icon: 'Globe2', audience: 'all' },
  { label: 'Demo Web', path: '/demo-web', icon: 'Globe2', audience: 'all' },
  { label: 'WhatsApp', path: '/whatsapp', icon: 'Phone', audience: 'all' },
  { label: 'Instagram', path: '/instagram', icon: 'Instagram', audience: 'all' },
  { label: 'TikTok', path: '/tiktok', icon: 'MessageSquare', audience: 'all' },
  { label: 'App Chat', path: '/app-chat', icon: 'Bot', audience: 'all' },
  ...
  { label: 'Campanas', path: '/campaigns', icon: 'Megaphone', audience: 'all' },
  { label: 'Flujos', path: '/flows', icon: 'GitBranch', audience: 'all' },
  { label: 'Productos', path: '/products', icon: 'Package', audience: 'all' },
  { label: 'Inventario', path: '/inventory', icon: 'Boxes', audience: 'all' },
  { label: 'Pedidos', path: '/orders', icon: 'ShoppingCart', audience: 'all' },
  { label: 'Overview', path: '/workspace/overview', icon: 'Brain', audience: 'all' },
  { label: 'Shared Memory', path: '/workspace/memory', icon: 'BookOpen', audience: 'all' },
  { label: 'Tasks', path: '/workspace/tasks', icon: 'GitBranch', audience: 'all' },
  { label: 'Insights', path: '/workspace/insights', icon: 'BarChart3', audience: 'all' },
  { label: 'Activity', path: '/workspace/activity', icon: 'Users', audience: 'all' },
  { label: 'Decisions', path: '/workspace/decisions', icon: 'Shuffle', audience: 'all' },
  { label: 'Integraciones', path: '/integrations', icon: 'Plug', audience: 'all' },
  { label: 'Configuracion', path: '/settings', icon: 'Settings', audience: 'all' },
  { label: 'Facturacion', path: '/billing', icon: 'CreditCard', audience: 'all' },
...
export default function MainLayout() {
  ...
  const sidebar = (
    <div className="flex flex-col h-full bg-white border-r border-ink-200">
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
        ...
        {isAdmin && (
          <div>
            <p className="text-xs font-semibold text-ink-400 uppercase tracking-wider px-3 mb-1.5">Administracion</p>
            <div className="space-y-0.5">
              {adminItems.map((item) => (
                <NavItem key={item.path} item={item} onClose={() => setSidebarOpen(false)} />
              ))}
  ...
  return (
    <div className="flex h-screen bg-ink-50 overflow-hidden">
      <div className="hidden lg:flex w-56 shrink-0 flex-col">{sidebar}</div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-14 bg-white border-b border-ink-200 flex items-center px-4 gap-3 shrink-0">
          <button className="lg:hidden p-1.5 text-ink-500 hover:text-ink-800" onClick={() => setSidebarOpen(true)}>
            <Menu size={20} />
          </button>
          ...
          <div className="relative" ref={orgRef}>
            ...
            {orgOpen && (
              <div className="absolute top-full left-0 mt-1.5 w-52 bg-white border border-ink-200 rounded-xl shadow-lg py-1.5 z-50">
                <p className="text-xs text-ink-400 px-3 pb-1.5 font-medium">Cambiar organizacion</p>
                {MOCK_ORGS.map((org) => (
                  <button
                    key={org.id}
                    onClick={() => {
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2 hover:bg-ink-50 transition-colors text-left ${
                      currentOrg.id === org.id ? 'bg-brand-50' : ''
                    }`}
                  >
