...
const WEBCHAT_ORG_SLUG = import.meta.env.VITE_WEBCHAT_ORG_SLUG || 'comfaguajira';
...
function TypingIndicator() {
  return (
    <div className="flex items-end gap-2">
      ...
      <div className="flex items-center gap-1 rounded-2xl rounded-bl-sm bg-[rgba(17,17,16,0.06)] px-4 py-3">
        {[0, 1, 2].map((i) => (
          <span
            key={i}
...
function ChatWindow({ onClose }: { onClose: () => void }) {
  ...
  if (!sessionIdRef.current) {
    const existing = window.localStorage.getItem('zelora_webchat_session');
    if (existing) {
      sessionIdRef.current = existing;
    } else {
      sessionIdRef.current = `web-${Date.now()}`;
      window.localStorage.setItem('zelora_webchat_session', sessionIdRef.current);
    }
  }
  ...
  async function submitToBackend(content: string) {
    if (USE_MOCK_DATA) return false;
    try {
      setBackendError(null);
      const response = await api.sendWebChatMessage({
        organization_slug: WEBCHAT_ORG_SLUG,
      ...
      setBackendError('El backend del canal web no respondio. No se mostraran datos mock en modo real.');
      return false;
    }
  }

  async function sendQuickReply(key: string) {
    const sent = await submitToBackend(USER_MESSAGES[key]);
    if (sent || !USE_MOCK_DATA) return;
  ...
  async function sendInput() {
    ...
    setInputVal('');
    const sent = await submitToBackend(content);
    if (sent || !USE_MOCK_DATA) return;
    const userMsg: ChatMessage = {
      id: `u-${Date.now()}`,
      role: 'user',
  ...
  return (
    <motion.div
      ...
      className="flex h-[520px] w-[360px] flex-col overflow-hidden rounded-2xl border border-[rgba(17,17,16,0.09)] bg-white/70 backdrop-blur-sm shadow-2xl"
    >
      {/* Header */}
      ...
      <div className="flex-1 space-y-3 overflow-y-auto bg-[rgba(17,17,16,0.025)] p-4">
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            ...
            >
              {msg.role !== 'user' && (
                <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-brand-500 text-white">
                  <Bot size={14} />
                </div>
              )}
              <div
                className={`max-w-[80%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed ${
                  msg.role === 'user'
                    ...
                    : 'rounded-bl-sm bg-white/70 backdrop-blur-sm text-ink-800 shadow-card border border-[rgba(17,17,16,0.06)]'
                }`}
              >
                {msg.content}
              </div>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {QUICK_REPLIES.map((qr) => (
              <button
                ...
                className="rounded-full border border-brand-200 bg-white/70 backdrop-blur-sm px-3 py-1.5 text-xs font-medium text-brand-700 hover:bg-brand-50 transition"
              >
                {qr.label}
              </button>
            ))}
      ...
      <div className="border-t border-[rgba(17,17,16,0.06)] bg-white/70 backdrop-blur-sm p-3">
        ...
        <div className="flex items-center gap-2 rounded-xl border border-[rgba(17,17,16,0.09)] bg-[rgba(17,17,16,0.025)] px-3 py-2">
          ...
          <button
            ...
            className="flex-shrink-0 rounded-lg p-1 text-ink-400 hover:bg-[rgba(17,17,16,0.08)] hover:text-ink-600 transition"
          ...
          <button
            ...
            className={`flex-shrink-0 flex h-8 w-8 items-center justify-center rounded-lg transition ${
              recording
                ...
                : 'text-ink-400 hover:bg-[rgba(17,17,16,0.08)] hover:text-ink-600'
            }`}
          ...
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-lg bg-brand-500 text-white transition hover:bg-brand-600 disabled:opacity-40"
          >
            <Send size={14} />
          </button>
        </div>
...
export function DemoWebPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="relative min-h-[600px] overflow-hidden rounded-2xl border border-[rgba(255,255,255,0.55)] border-b-[rgba(17,17,16,0.07)] bg-white/65 backdrop-blur-md shadow-card">
        {/* Fake nav */}
        <header className="flex items-center justify-between border-b border-[rgba(17,17,16,0.06)] bg-brand-600 px-6 py-3 text-white">
        ...
        <div className="bg-gradient-to-br from-brand-600 to-brand-500 px-6 py-14 text-white">
          <div className="mx-auto max-w-xl text-center">
            ...
            <div className="mt-6 flex justify-center gap-3">
              <button className="rounded-xl bg-white/70 backdrop-blur-sm px-5 py-2.5 text-sm font-bold text-brand-700 shadow hover:bg-brand-50 transition">
                Consultar subsidio
              </button>
        ...
        <div className="grid gap-4 px-6 py-8 md:grid-cols-3">
          {[
            ...
            <div
              key={svc.title}
              className="cursor-pointer rounded-2xl border border-[rgba(17,17,16,0.06)] bg-[rgba(17,17,16,0.025)] p-5 shadow-card transition hover:shadow-md hover:border-brand-200"
            >
              <span className="text-3xl">{svc.emoji}</span>
              <h3 className="mt-3 font-bold text-ink-900">{svc.title}</h3>
              <p className="mt-1 text-sm text-ink-400">{svc.desc}</p>
        ...
        <div className="absolute bottom-6 right-6 flex flex-col items-end gap-3">
          ...
          <motion.button
            ...
            className="flex h-14 w-14 items-center justify-center rounded-full bg-brand-500 text-white shadow-xl hover:bg-brand-600 transition"
          >
